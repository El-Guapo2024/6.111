...
void YDLidar::setIntensity(int i) {
  intensity = i;
  //Reallocate memory when setting intensity bits
  if (packageBuffer)
  {
    delete[] packageBuffer;
    packageBuffer = NULL;
...
void YDLidar::setSingleChannel(bool yes) {
  ...
  if (yes)
  {
    //Single-pass radar needs to be started through dtr control
...
result_t YDLidar::startScan(bool force, uint32_t timeout) 
{
  ...
  //Dual-pass radar starts scanning via command control
  if (!singleChannel)
  {
  ...
  else //The single-pass radar is started through serial port dtr control
  {
    //start motor in 1.8v
    setMotorSpeed(1.8);
